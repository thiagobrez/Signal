...
struct NotchView<Expanded, CompactLeading, CompactTrailing>: View where Expanded: View, CompactLeading: View, CompactTrailing: View {
    ...
    // [Signal patch] Cosmetic inset between the hosted card and the notch body on
    // the bottom/leading/trailing edges (upstream 15). Zeroed to match the same
    // patch in NotchlessView, so the card fills the black background to its border
    // on both notch and notchless screens — this keeps the confetti, which is an
    // overlay bounded by the card, reaching the edge consistently across displays.
    // The top edge is unaffected: it uses `notchSize.height` for physical-notch
    // clearance, not this constant.
    private let safeAreaInset: CGFloat = 0
